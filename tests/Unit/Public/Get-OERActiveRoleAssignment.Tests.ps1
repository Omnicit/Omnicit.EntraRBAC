...
Describe 'Get-OERActiveRoleAssignment' {
    ...
    BeforeAll {
        ...
        Mock -ModuleName Omnicit.EntraRBAC Invoke-OERArmRequest {
            [PSCustomObject]@{ value = @([PSCustomObject]@{ id='/subscriptions/s1/providers/Microsoft.Authorization/roleAssignmentSchedules/sch1'; name='sch1'
                properties=[PSCustomObject]@{ scope='/subscriptions/s1'; roleDefinitionId='rd1'; principalId='p1'; principalType='User'; status='Provisioned'; memberType='Direct'; assignmentType='Assigned'
                    startDateTime='t'; endDateTime='t2'; createdOn='t'; updatedOn='t'; roleAssignmentScheduleRequestId='req1'
    ...
    It 'GETs roleAssignmentSchedules with -All and no filter by default' {
        ...
        Should -Invoke -ModuleName Omnicit.EntraRBAC Invoke-OERArmRequest -Times 1 -ParameterFilter {
            $Path -eq '/subscriptions/s1/providers/Microsoft.Authorization/roleAssignmentSchedules?api-version=2020-10-01' -and $All
        }
    }
    It 'applies assignedTo() filter embedding the resolved principal id (transitive, includes group-inherited)' {
        ...
        Get-OERActiveRoleAssignment -Subscription 'Prod' -User 'anna@contoso.com' | Out-Null
        Should -Invoke -ModuleName Omnicit.EntraRBAC Invoke-OERArmRequest -Times 1 -ParameterFilter {
            $Path -like "*assignedTo*pid-1*" -and $Path -notlike '*principalId*'
        }
    }
    It 'applies atScope() for -AtScope' {
        Get-OERActiveRoleAssignment -Subscription 'Prod' -AtScope
    ...
    It 'errors when a principal filter is combined with -AtScope' {
        Get-OERActiveRoleAssignment -Subscription 'Prod' -User 'a' -AtScope -ErrorVariable e -ErrorAction SilentlyContinue
        $e[0].FullyQualifiedErrorId | Should -Match 'AmbiguousFilter'
    }
    It 'errors when a principal filter is combined with -AsTarget' {
        Get-OERActiveRoleAssignment -Subscription 'Prod' -User 'a' -AsTarget -ErrorVariable e -ErrorAction SilentlyContinue
        $e[0].FullyQualifiedErrorId | Should -Match 'AmbiguousFilter'
    }
    It 'errors when -AtScope and -AsTarget are combined' {
        Get-OERActiveRoleAssignment -Subscription 'Prod' -AtScope -AsTarget -ErrorVariable e -ErrorAction SilentlyContinue
        $e[0].FullyQualifiedErrorId | Should -Match 'AmbiguousFilter'
    }
    It 'returns tagged ActiveRoleAssignment objects' {
        (Get-OERActiveRoleAssignment -Subscription 'Prod')[0].PSObject.TypeNames[0] | Should -Be 'Omnicit.EntraRBAC.ActiveRoleAssignment'
    }
    It 'forwards -ResourceType/-ResourceName to Resolve-OERScope' {
        Get-OERActiveRoleAssignment -Subscription 'Prod' -ResourceGroup 'rg1' -ResourceType 'Microsoft.Storage/storageAccounts' -ResourceName 'stg1'
    ...
    It 'binds -ResourceGroup from the pipeline by property name' {
        $RgParam = (Get-Command Get-OERActiveRoleAssignment).Parameters['ResourceGroup']
    ...
    It 'binds -ResourceName and -ResourceType from the pipeline by property name' {
        foreach ($ParamName in 'ResourceName', 'ResourceType') {
            $Param = (Get-Command Get-OERActiveRoleAssignment).Parameters[$ParamName]
    ...
    It 'binds a piped resource-group-shaped object end to end (not just by attribute metadata)' {
        ...
        $Rg | Get-OERActiveRoleAssignment
        Should -Invoke -ModuleName Omnicit.EntraRBAC Resolve-OERScope -Times 1 -ParameterFilter {
    ...
    It 'binds a piped resource-shaped object end to end, including -ResourceName and -ResourceType' {
        ...
        $Resource | Get-OERActiveRoleAssignment
        Should -Invoke -ModuleName Omnicit.EntraRBAC Resolve-OERScope -Times 1 -ParameterFilter {
    ...
    It 'scrubs the bearer-hygiene record when the ARM transport fails' {
        ...
        # the roleAssignmentSchedules list catch.
        Mock -ModuleName Omnicit.EntraRBAC Resolve-OERScope { '/subscriptions/s1' }
        Mock -ModuleName Omnicit.EntraRBAC Invoke-OERArmRequest { throw 'transport failure' }
        Mock -ModuleName Omnicit.EntraRBAC Remove-OERErrorRecord { }
        $null = Get-OERActiveRoleAssignment -Subscription 'Prod' -ErrorAction SilentlyContinue
