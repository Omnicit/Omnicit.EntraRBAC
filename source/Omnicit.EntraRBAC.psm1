...
# ModuleBuilder merges all Private/ and Public/ .ps1 files (and Classes/, if it
# is ever added) into a
# single Omnicit.EntraRBAC.psm1, prepends any `using namespace` statements, and
...
# Load order: Classes -> Private (helpers) -> Public (exported functions). Classes/ does not currently
# exist -- it is iterated only so that adding it later needs no loader change. Argument completion is
# NOT class-based: see the Register-ArgumentCompleter blocks at the bottom of this file (mirrored in
# suffix.ps1 for the built module).
$PublicFunctions = [System.Collections.Generic.List[string]]::new()
foreach ($ScriptPathItem in 'Classes', 'Private', 'Public') {
    $ScriptFolder = [io.path]::Combine($PSScriptRoot, $ScriptPathItem)
    # Classes/ is optional and currently absent (completion is Register-ArgumentCompleter based, not
    # IArgumentCompleter classes); skip a folder that does not exist so Get-ChildItem does not emit an
    # ItemNotFound error on import.
    if (-not (Test-Path -LiteralPath $ScriptFolder)) { continue }
    $ScriptSearchFilter = [io.path]::Combine($ScriptFolder, '*.ps1')
...
# Omnicit.EntraRBAC.* custom types - no Az-native type overrides remain, so the AppendPath precedence
