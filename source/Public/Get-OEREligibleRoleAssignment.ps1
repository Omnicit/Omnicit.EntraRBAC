function Get-OEREligibleRoleAssignment {
    <#
    .SYNOPSIS
    Lists eligible Azure role assignments (PIM) at a management group, subscription, resource group,
    or resource scope.
    .DESCRIPTION
    Reads Microsoft.Authorization/roleEligibilitySchedules through the ARM API (api-version
    ...
    display name). Without a filter, every eligibility that applies at the scope (direct and
    inherited) is returned. A principal filter (-User, -Group, or -ServicePrincipal; resolved through
    Microsoft Graph) applies $filter=assignedTo('<objectId>') so eligibilities inherited via group membership
    are included. -AtScope applies $filter=atScope()
    (eligibilities at or above the scope). -AsTarget applies $filter=asTarget() (the caller's own
    eligibilities). A principal filter, -AtScope, and -AsTarget are mutually exclusive. Output objects
    are tagged Omnicit.EntraRBAC.EligibleRoleAssignment and expose RoleEligibilityScheduleId,
    PrincipalId, RoleDefinitionId, and Scope so they pipe into Remove-OEREligibleRoleAssignment and
    Enable-OEREligibleRoleAssignment. Friendly RoleName/PrincipalDisplayName are returned directly
    from the API. Requires an ARM token; authentication is ensured via Initialize-OERAuth -IncludeARM.
    .PARAMETER Scope
    ...
    .PARAMETER User
    A user principal name or object id whose eligibilities are listed.
    .PARAMETER Group
    A group display name or object id whose eligibilities are listed.
    .PARAMETER ServicePrincipal
    A service principal display name or object id whose eligibilities are listed.
    .PARAMETER AtScope
    Return only eligibilities at or above the scope. Cannot be combined with a principal filter or -AsTarget.
    .PARAMETER AsTarget
    Return only the calling user's own eligibilities. Cannot be combined with a principal filter or -AtScope.
    .PARAMETER TenantId
    Optional tenant id or domain to authenticate against, forwarded to Initialize-OERAuth.
    .EXAMPLE
    Get-OEREligibleRoleAssignment -Subscription 'Prod'
    Lists every eligible role assignment that applies at the Prod subscription.
    .EXAMPLE
    Get-OEREligibleRoleAssignment -ManagementGroup 'mg-platform' -User 'anna.berg@contoso.com'
    Lists Anna's eligible role assignments at, above, and below the mg-platform management group.
    .EXAMPLE
    Get-OEREligibleRoleAssignment -Subscription 'Prod' -AsTarget
    Lists the caller's own eligible role assignments at the Prod subscription.
    #>
    [CmdletBinding()]
    ...
    process {
        ...
        if ($PrincipalRequested) {
            ...
            # eligibilities (verified against the ARM 2020-10-01 _ListForScope filter contract).
        ...
        $Path = "$TargetScope/providers/Microsoft.Authorization/roleEligibilitySchedules?api-version=2020-10-01"
